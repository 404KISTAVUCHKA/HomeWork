...
/// Слайдер для offers (постраничная прокрутка, две строки)
const offersWrapper = document.querySelector('.offers__swiper-wrapper');
const prevBtn = document.querySelector('.offers__button-prev');
const nextBtn = document.querySelector('.offers__button-next');
const pageNumSpan = document.querySelector('.offers__num-page .num-page');

if (offersWrapper && prevBtn && nextBtn && pageNumSpan) {
  const updateSliderState = () => {
    const containerWidth = offersWrapper.clientWidth;
    const scrollWidth = offersWrapper.scrollWidth;
    const maxScrollLeft = scrollWidth - containerWidth;
    const currentScroll = offersWrapper.scrollLeft;

    // Количество страниц = общая ширина / ширина контейнера (округляем вверх)
    const totalPages = Math.max(1, Math.ceil(scrollWidth / containerWidth));
    // Текущая страница (с учётом возможной нецелой прокрутки)
    let currentPage = Math.floor((currentScroll + containerWidth / 2) / containerWidth) + 1;
    currentPage = Math.min(currentPage, totalPages);

    pageNumSpan.textContent = `${currentPage} из ${totalPages}`;

    // Блокируем кнопки при достижении краёв
    prevBtn.disabled = currentScroll <= 1;
    nextBtn.disabled = currentScroll >= maxScrollLeft - 1;
  };

  // Подписываемся на прокрутку
  offersWrapper.addEventListener('scroll', () => {
    requestAnimationFrame(updateSliderState);
  });

  // Обработчики кликов с плавной прокруткой
  prevBtn.addEventListener('click', () => {
    offersWrapper.scrollBy({
      left: -offersWrapper.clientWidth,
      behavior: 'smooth'
    });
  });
...
(function initLightbox() {
  ...
  const reviewsContainer = document.querySelector('.review__feedback-container');
  if (reviewsContainer) {
